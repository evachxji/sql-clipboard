...
历史教训：早期会话把全屏截图直接入库，桌面图标、微信聊天窗口、业务表格（社保卡数据）等隐私内容随之泄露并推送到 GitHub，最终只能重写 git 历史清洗。所有截图必须遵守：

- **禁止全屏截图**：只截应用窗口本身（Win11 截图工具选「窗口」模式，或脚本按窗口矩形截取）
- **入库前逐张回看**：画面不得包含桌面、任务栏、其他软件窗口、聊天/邮件内容、含个人信息的文件路径
- **敏感信息脱敏**：JDBC URL、IP、真实业务数据需裁剪或打码
- 截图一经提交即视为公开内容，宁缺毋滥
